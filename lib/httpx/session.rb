  ...
  class Session
    ...
    def close(selector = Selector.new)
      ...
      begin
        ...
        next_tick(selector) until selector.empty?
      ensure
        @closing = false
      end
    end
    ...
    def receive_requests(requests, selector)
      ...
      loop do
        ...
        catch(:coalesced) { next_tick(selector) } until (response = fetch_response(request, selector, request.options))
    ...
    def next_tick(selector)
      catch(:jump_tick) do
        timeout = selector.next_timeout
        if timeout && timeout.negative?
          selector.timers.fire
          throw(:jump_tick)
        end
        ...
        begin
          ...
          selector.timers.fire(e)
        end
      end
    rescue StandardError => e
      selector.emit_error(e)
    rescue Exception # rubocop:disable Lint/RescueException
      selector.each_connection(&:force_reset)
      raise
    end
