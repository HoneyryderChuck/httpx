...
module WebMock
  module HttpLibAdapters
    ...
    HTTPXOriginalSession = HTTPX::Session unless const_defined?(:HTTPXOriginalSession)

    class HTTPXMockSession < HTTPX::Session
      private
    ...
    class HttpxAdapter < HttpLibAdapter
      ...
      class << self
        def enable!
          ...
          HTTPX.send(:const_set, :Session, HTTPXMockSession)
        end

        def disable!
          HTTPX.send(:remove_const, :Session)
          HTTPX.send(:const_set, :Session, HTTPXOriginalSession)
        end
      end
    end
  end
